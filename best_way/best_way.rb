...
class BestWay
  ...
  def choose_first_points
    first_points = @first_position.dig(@first_position.keys*"").sort_by { |k, v| [v, k] }.first
  end
  ...
  def choose_second_points
    ...
    two_points = second_position.first.dig(second_position.first.keys*"").sort_by { |k, v| [v, k] }.first
  end
